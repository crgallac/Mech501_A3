...
%    selectDegeneracyScheme =   Choose scheme for degenerate linear constraints
...
gui_var.newMatlab   = str2double(version('-release')) ...
                   >= gui_var.Defaults.Labels.newMATLAB;
...
guiFunction = { ...
   ...
   'updateScreen','selectDegeneracyScheme', ...
   'edit_gui','search_gui','dace_gui','nw_gui','rbf_gui','results_gui', ...
   'solution_gui','loadEdit','loadSearchOptions','loadSurrogateOptions', ...
   'loadUserSearchFile','changeView','modifySearchScreen','updateSearchLabels'};
...
if gui_var.newMatlab
   ...
   gui.FilterPanel = uipanel('Parent',gui.fig, ...
                             'Position',[panelLeft, .295, panelWidth, .13], ...
                             'BorderType','beveledout');
   gui.RSPanel     = uipanel('Parent',gui.fig, ...
                             'Position',[panelLeft, .075, panelWidth, .22], ...
...
for k = 1:length(gui_var.Labels.Parameters.RS)-2
   gui.RSLabel(k) = uicontrol(gui.fig, ...
                  ...
                  'Position',[labelLeft,.29-.03*k,labelLongWidth,textHeight],...
                  'Visible', 'off');
end
gui.RSTermFlag.alpha = uicontrol(gui.fig, ...
      'String',        gui_var.Labels.Parameters.RS{end-1}, ...
      'Style',         'checkbox', ...
      'Position',      [labelLeft, .11, labelLongWidth, textHeight], ...
      'Visible',       'off');
...
set(gui.TermFlag.delta,'ToolTipString',...
    'MADS terminates when the mesh size becomes smaller than this value');
...
for k = 1:length(gui_var.Labels.degeneracyScheme)
   gui.degeneracyMenu(k) = uimenu(gui.OptionsMenu.degeneracyMenu, ...
      ...
      'Callback', gui.func.selectDegeneracyScheme);
end
...
gui.MADSMenu.pollStrategies    = uimenu(gui.Menu.MADS, 'Position', 2, ...
   'Separator',   'on', ...
   'Label',       'Select Poll Directions');
gui.MADSMenu.pollOrders        = uimenu(gui.Menu.MADS, 'Position', 3, ...
   'Label',       'Select Poll Order');
gui.MADSMenu.pollCenters       = uimenu(gui.Menu.MADS, 'Position', 4, ...
   'Label',       'Select Poll Center');
gui.MADSMenu.pollCompleteFlags = uimenu(gui.Menu.MADS, 'Position', 5, ...
   'Label',       'Complete Polling');
gui.MADSMenu.editTermParam     = uimenu(gui.Menu.MADS, 'Position', 6, ...
   'Separator',   'on', ...
...
gui.MADSMenu.editMeshParam     = uimenu(gui.Menu.MADS, 'Position', 7, ...
   ...
   'Callback',    gui.func.edit_gui);
gui.MADSMenu.editOtherParam    = uimenu(gui.Menu.MADS, 'Position', 8, ...
   'Label',       'Edit Filter/MVP Parameters', ...
   'UserData',    {gui_var.Labels.Parameters.other, ...
                  [gui.hmin; gui.hmax; gui.ePollXiF; gui.ePollXiH]}, ...
   'Callback',    gui.func.edit_gui);
gui.MADSMenu.editRSParam       = uimenu(gui.Menu.MADS, 'Position', 9, ...
...
for k = 1:length(gui_var.Labels.pollStrategy)
   gui.MADSMenu.pollStrategy(k) = uimenu(gui.MADSMenu.pollStrategies, ...
      ...
      'Callback', {gui.func.selectPoll,'Strategy'});
   if strncmp(gui_var.Types.poll{k}, 'Gradient', 8)
      set(gui.MADSMenu.pollStrategy(k),'Enable','off');
   end
   if strncmp(gui_var.Types.poll{k}, 'MADS_SP', 7)
      if strcmp(get(gui.OptionsMenu.runSensor,'Checked'),'off')
         set(gui.MADSMenu.pollStrategy(k),'Visible','off');
      end
   end
end
for k = 1:length(gui_var.Labels.pollCenter)
   gui.MADSMenu.pollCenter(k) = uimenu(gui.MADSMenu.pollCenters, ...
      ...
      'Callback', {gui.func.selectPoll,'Center'});
end
for k = 1:length(gui_var.Labels.pollOrder)
   gui.MADSMenu.pollOrder(k) = uimenu(gui.MADSMenu.pollOrders, ...
      ...
      'Callback', {gui.func.selectPoll,'Order'});
end
gui.MADSMenu.pollComplete   = uimenu(gui.MADSMenu.pollCompleteFlags, ...
   'Position', 1, ...
   'Label',       'Poll Step', ...
   'Callback',    'umtoggle(gcbo);');
gui.MADSMenu.NPollComplete  = uimenu(gui.MADSMenu.pollCompleteFlags, ...
   'Position', 2, ...
   'Label',       'Discrete Neighbor Poll', ...
   'Callback',    'umtoggle(gcbo);');
gui.MADSMenu.EPollComplete  = uimenu(gui.MADSMenu.pollCompleteFlags, ...
   'Position', 3, ...
   'Label',       'Extended Poll', ...
   'Callback',    'umtoggle(gcbo);');
...
uimenu(gui.Menu.Help,'Position',7, ...
   ...
   'Callback',   gui.func.getHelp);
uimenu(gui.Menu.Help,'Position',8, ...
   'Separator',  'on', ...
   'Label',      'About NOMADm', ...
...
if (pFile)
   ...
   set(gui.MADSMenu.pollStrategy(pLabels(:,1) == 'G'), ...
...
%*******************************************************************************
% selectDegeneracyScheme: Select scheme for handling degeneracy.
% ------------------------------------------------------------------------------
% Called by: nomadm_gui (callback: 
%            Options-->Scheme for Handling Degenerate Linear Constraints) 
% VARIABLES:
%  h              = handle of calling object
%  gui_var.Choice = user choices
%  gui            = structure of NOMADm object handles
%*******************************************************************************
function selectDegeneracyScheme(h,event)

% Set checkboxes and update degeneracy scheme Choice variable
set(get(get(h,'Parent'),'Children'), 'Checked','off');
umtoggle(h);
gui_var = getappdata(gcbf,'gui_var');
gui_var.Choice.degeneracyScheme = get(h,'Position');
setappdata(gcbf,'gui_var',gui_var);
return
...
umtoggle(gui.filterMenu(2));
umtoggle(gui.filterMenu(3));
umtoggle(gui.sensorMenu(2));
umtoggle(gui.sensorMenu(3));
umtoggle(gui.sensorMenu(4));
umtoggle(gui.debugMenu(2));
umtoggle(gui.debugMenu(3));
umtoggle(gui.debugMenu(4));
umtoggle(gui.plotHistoryMenu(2));
umtoggle(gui.plotHistoryMenu(3));
umtoggle(gui.CacheMenu.load);
umtoggle(gui.CacheMenu.count);
umtoggle(gui.MADSMenu.pollComplete);
umtoggle(gui.MADSMenu.NPollComplete);
umtoggle(gui.MADSMenu.EPollComplete);
...
umtoggle(gui.OptionsMenu.saveHistory);
...
Options.degeneracyScheme  = gui_var.Types.degeneracyScheme{...
                            gui_var.Choice.degeneracyScheme};
Options.pollComplete      = umtoggle(gui.MADSMenu.pollComplete);
Options.NPollComplete     = umtoggle(gui.MADSMenu.NPollComplete);
Options.EPollComplete     = umtoggle(gui.MADSMenu.EPollComplete);
...
Options.useFilter         =  1*umtoggle(gui.filterMenu(2)) + ...
                             2*umtoggle(gui.filterMenu(3));
Options.scale             =  2*umtoggle(gui.scaleMenu(2)) + ...
                            10*umtoggle(gui.scaleMenu(3));
Options.dimSensor         =  1*umtoggle(gui.sensorMenu(2)) + ...
                             2*umtoggle(gui.sensorMenu(3)) + ...
                             3*umtoggle(gui.sensorMenu(4));
Options.debug             =  1*umtoggle(gui.debugMenu(2)) + ...
                             2*umtoggle(gui.debugMenu(3)) + ...
                             3*umtoggle(gui.debugMenu(4));
Options.plotHistory       =  1*umtoggle(gui.plotHistoryMenu(2)) + ...
                             2*umtoggle(gui.plotHistoryMenu(3));
Options.plotFilter        = umtoggle(gui.OptionsMenu.plotFilter);
Options.saveHistory       = umtoggle(gui.OptionsMenu.saveHistory);
...
phandles = [guiSearch.Labels.param; [guiSearch.Screen(1:nSearches).param]'];

set(shandles,'Visible',onoff{2-any(hasSFile(1:nSearches))});
set(uhandles,'Visible',onoff{2-any(hasUFile(1:nSearches))});
set(phandles,'Visible',onoff{2-any(isSurrogate(1:nSearches))});
...
% Calls:     dace_gui, nw_gui, rbf_gui, modifySearchScreen
% VARIABLES:
%  fig                   = temporary storage of figure handles
%  gui_var               = structure of all GUI variables
...
set(gui.ePollXiF,    'String', num2str(Options.ePollTriggerF,'%3.5g'));
set(gui.ePollXiH,    'String', num2str(Options.ePollTriggerH,'%3.5g'));
...
for k = 1:length(gui.MADSMenu.pollStrategy)
   set(gui.MADSMenu.pollStrategy(k),'Checked', ...
   onoff{1+~(Choice.pollStrategy==k)});
end
for k = 1:length(gui.MADSMenu.pollOrder)
   set(gui.MADSMenu.pollOrder(k),'Checked', ...
   onoff{1+~(Choice.pollOrder==k)});
end
for k = 1:length(gui.MADSMenu.pollCenter)
   set(gui.MADSMenu.pollCenter(k),'Checked', ...
   onoff{1+~(Choice.pollCenter==k)});
end
...
set(gui.degeneracyMenu(1), 'Checked',onoff{1+~(Choice.degeneracyScheme == 1)});
set(gui.degeneracyMenu(2), 'Checked',onoff{1+~(Choice.degeneracyScheme == 2)});
set(gui.degeneracyMenu(3), 'Checked',onoff{1+~(Choice.degeneracyScheme == 3)});
set(gui.degeneracyMenu(4), 'Checked',onoff{1+~(Choice.degeneracyScheme == 4)});
...
set(gui.MADSMenu.pollComplete,  'Checked',onoff{2-Options.pollComplete});
set(gui.MADSMenu.NPollComplete, 'Checked',onoff{2-Options.NPollComplete});
set(gui.MADSMenu.EPollComplete, 'Checked',onoff{2-Options.EPollComplete});
...
for k = 1:nSearches
   if isfield(Search(k),'sfile')
      [ignore,sFile] = fileparts(Search(k).sfile);
   end
