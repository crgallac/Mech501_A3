...
% Adjust choices for MADS that must hold for the theory
...
if isfield(Problem,'Param')
   if isfield(Problem.Param,'maxNp')
      Problem.maxNp = Problem.Param.maxNp;
   end
   if isfield(Problem.Param,'maxNx')
      Problem.maxNx = Problem.Param.maxNx;
   end
   if isfield(Problem.Param,'PollBasis')
      Problem.pollBasis = Problem.Param.pollBasis;
   end
   if isfield(Problem.Param,'PollOrder')
      RunData.porder = Problem.Param.pollOrder;
   end
end
...
if ~success || all(~isfinite([Cache.iterate(1:Cache.size).f]))
   err = []; iterate = [];
   for i = 1:length(iterate0)
      [iteratetemp,errtemp] = makeFeasible(Problem.iterate0(i),Problem.Omega);
      if errtemp == -9
         error('mads:missingToolbox', ...
               'Optimization Toolbox required (makeFeasible)');
      end
      iterate = [iterate, iteratetemp];
      err = [err,errtemp];
   end
   if all(err <= 0)
      error('mads:x0', ...
            'MADS cannot find a feasible starting point (makeFeasible)');
   end
   [iterate,success,TempFilter,RunData.RS] = ...
...
errorMsg{19}  = 'Lower bound on surrogate parameter exceeds its upper bound';
errorMsg{20}  = 'Number of R&S samples must be positive';
errorMsg{21}  = 'Initial Indifference Zone parameter must be > 0';
errorMsg{22}  = 'Initial Alpha parameter must be a number in (0,1)';
errorMsg{23}  = 'R&S Decay Factors must be numbers in (0,1)';
...
errorCheck(9)  = any(isnan(Term)) || any(Term <= 0);
errorCheck(10) = all(isinf(Term));
...
errorCheck(14) = ~isfinite(Options.meshCoarsen) || Options.meshCoarsen < 1;
...
for k = 1:Options.nSearches
   switch Options.Search(k).type
      case {'DACE','SPS-DACE'}
         errorCheck(19) = Options.dace(k).lower > Options.dace(k).upper;
      case {'NW','SPS-NW'}
         errorCheck(19) = Options.nw(k).lower > Options.nw(k).upper;
      otherwise
         errorCheck(19) = 0;
   end
   if errorCheck(19), break, end
end

if Options.runStochastic
   errorCheck(20) = ~isfinite(Options.RS.s0)       || Options.RS.s0 <= 0;
   errorCheck(21) = ~isfinite(Options.RS.iz_const) || Options.RS.iz_const <= 0;
   errorCheck(22) = ~isfinite(Options.RS.alpha_const) ...
                 || Options.RS.alpha_const <= 0 || Options.RS.alpha_const >= 1;
   errorCheck(23) = ~isfinite(Options.RS.iz_rho) ...
...
%      .param       =     an unspecified user-determined value
...
[iterate.x,ignore,err] = runLP(c,A,b);
iterate.x(end) = [];
return
...
case {'SPOLLI','GPOLLI'}
   ...
   Options.pollComplete = Search.complete;
...
case 'LHS'
   ...
   [Problem.Omega,scale] = updateOmega(Problem,Options,pcenter);
   n  = pcenter.n;
   LB = Problem.Omega.l(1:n);
   UB = Problem.Omega.u(1:n);

   % Error if any variable has non-finite bounds
   if ~isequal(Problem.Omega.A(1:n,1:n),eye(n)) || ~all(isfinite(LB.*UB))
   ...
   if exist(Problem.File.X,'file')
      ...
      if valid < min(.75*nPoints,nPoints-1)
...
case 'MESH'
   q  = ceil(Search.nPoints^(1/pcenter.n));
   Options.removeRedundancy = 0;
   [Problem.Omega,RunData.scale] = updateOmega(Problem,Options,pcenter);
...
case 'PS'
   ...
   [x,fx,termflag] = ps(Problem.File.F,[LB,UB],Options);
   for k = 1:length(fx)
     S(k).x = x(:,k);
     S(k).p = pcenter.p;
     S(k).f = fx(k);
...
[ignore,success,Filter] = evalPointSet(Problem,S,Search.complete,Options,...
                                       Filter,RunData.RS,2);
return
...
case {'MADS_2n','MADS_n+1'}
   [D,RunData.bSet] = madsDirections(pcenter.x,Options.pollStrategy, ...
                      RunData.delta,RunData.scale,RunData.bSet);
   if Options.dimSensor
      D = D(:,1);
      if mod(pcenter.n,Options.dimSensor)
         error('poll:mads:badSP','Sensor dimension does not divide into x.');
      end
      for k = 1:pcenter.n/Options.dimSensor
         ind = 1+(k-1)*Options.dimSensor : k*Options.dimSensor;
         sensorD = madsDirections(pcenter.x(ind),Options.pollStrategy, ...
                                  RunData.delta, RunData.scale(ind));
...
Cache = getappdata(0,Problem.nameCache);
full = Options.NPollComplete;
[N,success,TempFilter,RunData.RS] = evalPointSet(Problem,N,full,Options, ...
...
if ~success
   ...
   if ~isempty(ePollF)
      [ignore,order] = sort([N(ePollF).f]);
      ePollF = ePollF(order);
   end
   if ~isempty(ePollH)
      [ignore,order] = sort([N(ePollH).h]);
      ePollH = ePollH(order);
   end
   ...
   for k = 1:length(ePoll)
      ...
      [ignore,Filter] = updateFilter(pcenter,Filter,Cache);      
      if Problem.adaptOmega
         [Problem.Omega,RunData.scale] = updateOmega(Problem,Options,pcenter);
      end
      ...
      TempOptions.PollComplete = Options.EPollComplete;
      ...
      if ~success
         for j = startCache:endCache
            [ignore,Filter] = updateFilter(Cache.iterate(j),Filter,Cache);
         end
%         pcenter = getPollCenter(Options.pollCenter,Filter,Cache);
      ...
      while ~success
         startCache = Cache.size + 1;
         Options.pollComplete = Options.EPollComplete;
      ...
      if success && ~Options.EPollComplete, break; end
   end
end
return
...
[RunData.pcenter,iCache] = getPollCenter(Options.pollCenter,Cache.Filter,Cache);
...
if strcmp(Options.pollOrder,'SimplexGradient')
   RunData.sd = getSimplexGradient(100,Cache,iCache,RunData.delta,success);
end
...
%*******************************************************************************
function [xBestG,fBestG,termflag] = ps(f,B,Options)
...
%  n           = number of continuous variables
%  es_options  = options for GA optimizer
%  es,result   = GA optimizer output variables
...
case 'mads'
   ...
   [ignore,ignore,RunStats,SurCache] = mads(Sur.Problem,iterate,Sur.Options);
   termflag = RunStats.stopRun;

   % Retrieve and evaluate a number of points
   indFeasible = 1:min(nPoints,length(SurCache.Filter.feasible));
   indFilter   = 1:min(nPoints,length(SurCache.Filter.F));
...
case 'cmaes'
   ...
   [ignore,order] = sort(sf);         % Sort points by surrogate f-value
   S = S(order);
end
return
...
% VARIABLES:
%  D        = matrix whose columns form the positive spanning set
%  bSet     = matrix of previously random MADS vectors
%  x        = continuous variable values of the current iterate
%  strategy = code indicating chosen poll strategy
%  delta    = mesh size parameter
%  scale    = scale factors for scaling the appropriate Poll directions
...
%*******************************************************************************
function [D,bSet] = madsDirections(x,strategy,delta,scale,bSet)
...
if k+1 <= size(bSet,2)
   b = bSet(:,k+1);
   [ignore,ihat] = max(abs(b));
else

   % Get random direction b that must remain the same for a specified step size
   b = zeros(n,1);
   for i = 1:n
      ind = randperm(2*m-1);
      b(i) = -m + ind(1);
   ...
   ihat = ind(1);
   ind  = randperm(2);
   b(ihat) = sign(ind(1)-1.5)*m;
   bSet(:,end+1) = b;
end

%Construct basis matrix B from b and (n-1)x(n-1) lower triangular matrix L
L = zeros(n-1);
for i = 1:n-1
   for j = 1:i-1
      ind = randperm(2*m-1);
...
switch strategy
case {'MADS_2n','MADS_SP_2n'}
   D = [B -B];
case {'MADS_n+1','MADS_SP_n+1'}
   D = [B -sum(B,2)];
otherwise
...
if ~isempty(B)
   ...
   % If hmax is not exceeded, solve LP to get descent direction in f and cB
      ...
      [X,ignore,exitflag] = runLP(f,A,b,Aeq,beq,LB,UB);
      if exitflag == -9
         error('mads:missingToolbox', ...
...
while i <= m
   ...
   if ~NR
      ind    = 1:length(bb);
      ind(i) = [];
      [ignore,fval] = runLP(-AA(i,:)',AA(ind,:),bb(ind));
      if isempty(fval)
         NR = 1;
      else
         NR = (-fval > bb(i));
...
switch orderCode
   ...
   case 'Surrogate'
      ...
      if RunData.pcenter.h > 0
         [ignore,order] = sort(sh);
      else
         [ignore,order] = sort(sf);
      end
   case 'Custom'
...
if noFeasible && noFilter
   error('mads:pollCenter:badFilter', ...
         'No suitable poll center (PollCenter).');
end
...
if Options.removeRedundancy
   ...
   for i = 1:length(ind)
      indcopy = ind;
      indcopy(i) = [];
      [ignore,fval] = runLP(-AA(ind(i),:)',AA(indcopy,:),bb(indcopy));
      if isempty(fval)
         Redundant(ind(i)) = 0;
      else
...
[P.f,P.c,P.h,P.sf,P.sc,P.sh,P.param] = deal([]);
...
if Options.computeGrad
   [P.gradf,P.gradc,P.gradh,P.sgradf,P.sgradc,P.sgradh] = deal([]);
end
...
for k = 1:nP
   ...
   msg = ['   evalPointSet: p/x = {',p,'} / [',num2str(P(k).x'),']'];
   ...
   if inOmega(P(k),Problem.Omega)
      ...
      if hit
         dispDebug(3,Options.debug,[msg,' -- Cache hit']);
      else
         P(k) = evalFunction(Problem.File.F,Problem.File.X, Problem.fType, ...
                             P(k),Options.computeGrad);
         dispDebug(3,Options.debug,[msg, ',  f = ',num2str(P(k).f)]);
      ...
      dispDebug(3,Options.debug,[msg,' -- not in Omega']);
   end
end
Cache = getappdata(0,Problem.nameCache);
...
if Cache.size >= max(1024,length(Cache.iterate))
   ...
   [Cache.iterate(ind).param] = deal(0);
...
if Options.saveHistory
   ...
   [ignore,fType] = strtok(Problem.File.H,'.');
   if strcmp(fType,'mat')
...
% Solve LP problem if the Optimization Toolbox license is present
if license('test','Optimization_Toolbox')
   warning('off','all');
...
while ~ssmDone
   ...
   if length(I) == 1
      meanvalue = [meanvalue; I, mean(F(:,I)), s, std(F(:,I))];
      break
   end
